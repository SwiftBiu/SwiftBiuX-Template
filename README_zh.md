...
#### `script.js` 开发
您需要实现两个函数：
*   `isAvailable(context)`: (异步) 判断动作是否应在当前上下文显示。必须返回一个解析为 `{ isAvailable: Boolean, isContextMatch: Boolean }` 的 Promise。
*   `performAction(context)`: (异步) 当用户点击动作时执行。
...
当您的插件 UI 显示时，SwiftBiu 会向您的 JavaScript 上下文中注入一个强大的 `window.swiftBiu` 对象。所有异步方法都返回 **Promise**，因此您可以使用现代的 `async/await` 语法。
...
*   **`swiftBiu.fetch(url, options)`**: (异步) 发起一个网络请求。`options` 支持 `method`, `headers`, 和 `body`。
    *   **返回**: `Promise<{ status: Int, data: String }>`
    *   **示例**: `const response = await swiftBiu.fetch('https://api.example.com/data');`

*   **`swiftBiu.copyText(text)`**: (异步) 将给定的文本复制到系统剪贴板。

*   **`swiftBiu.closeWindow()`**: (同步) 关闭当前的插件 UI 窗口。

#### 3. UI 控制与最佳实践

*   **`swiftBiu.ui.resizeWindow({ height: Number })`**: (异步) 调整插件窗口的高度。这对于创建内容动态变化的 UI至关重要。
...
*   **`swiftBiu.storage.get(key)`**: (异步) 从您的插件配置中读取一个值。`key` 必须与 `manifest.json` 中定义的一个相匹配。
