...
2.  **UI 脚本 (`ui/index.html`)**:
    ...
    *   **关键 API**: `window.swiftBiu.copyText()`, `window.swiftBiu.exportFile()`, `window.swiftBiu.openURL()`, `window.swiftBiu.pasteText()`, `window.swiftBiu.closeWindow()`, `window.swiftBiu.ui.resizeWindow()` 等。
...
*   **`swiftBiu.openURL(url)`**: 在用户的默认浏览器中打开指定的 URL。
    *   **示例**: `swiftBiu.openURL('https://www.google.com');`

*   **`swiftBiu.pasteText(text)`**: 将文本写入剪贴板，然后将其粘贴到用户之前活跃的应用程序中。在粘贴操作期间，插件窗口保持可见（浮动）。
    *   **权限**: 需要在 `manifest.json` 中声明 `"paste"` 权限。
    *   **示例**: `swiftBiu.pasteText('来自插件的问候！');`
...
*   **`window.swiftBiu.exportFile(base64, filename)`**: 触发 macOS 原生保存对话框。允许用户将插件生成的数据（Base64 格式）保存为本地文件。

#### 6. 原生文件与文件夹选择
插件现在可以直接从 UI 触发 macOS 原生的文件和文件夹选择对话框。

*   **文件选择**: 使用标准的 `<input type="file">`。
*   **文件夹选择**: 添加 `webkitdirectory` 属性：`<input type="file" webkitdirectory>`。这对于 **插件打包器 (Plugin Packager)** 等工具非常有用。

**文件处理最佳实践：**
在处理大量文件或执行重复操作（如打包）时，避免长时间依赖同一个 `File` 对象。建议在用户选择文件后，立即使用 `FileReader` API 将文件内容读取为 `ArrayBuffer` 或 `DataURL` 并进行缓存，以确保后续操作的稳定性。
