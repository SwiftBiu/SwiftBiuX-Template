...
*   `"localFileRead"`: `SwiftBiu.getFileMetadata(path)`、`SwiftBiu.readLocalFile(path)`、`SwiftBiu.readLocalTextFile(path)`、`SwiftBiu.listDirectory(path)`、`SwiftBiu.openFileWithApp(path, appBundleID)`、`window.swiftBiu.getFileMetadata(path)`、`window.swiftBiu.readLocalFile(path)`、`window.swiftBiu.readLocalTextFile(path)`、`window.swiftBiu.listDirectory(path)` 与 `window.swiftBiu.openFileWithApp(path, appBundleID)` 需要此权限 — 允许插件检查、读取或打开当前所选本地文件或可访问目录。
