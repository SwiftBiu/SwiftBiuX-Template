...
| :---: | :---: | :--- | :--- | :--- |
| `[x]` | `⚡️` | **cny** | 数字转人民币大写。 | 官方 |
| `[ ]` | `⚡️` | **JSON 格式化** | 美化和验证 JSON 字符串。 | 官方 |
| `[ ]` | `⚡️` | **Base64 编解码** | Base64 编码与解码。 | 官方 |
| `[ ]` | `⚡️` | **单词/字符统计** | 统计选中内容的单词数、字符数、行数等。 | 官方 |
| `[ ]` | `⚡️` | **时间戳转换** | 时间戳和日期时间格式互相转换。 | 官方 |
| `[ ]` | `⚡️` | **大小写转换** | 英文字母大小写转换 (Upper, Lower, Camel, Snake)。 | 官方 |
| `[ ]` | `⚡️` | **文本清洗工** | 去除空行/首尾空格、全角转半角、行去重、排序等。 | 官方 |
| `[ ]` | `⚡️` | **正则提取器** | 使用正则表达式批量提取文本中的关键信息（如邮箱、URL）。 | 官方 |
| `[ ]` | `⚡️` | **Markdown 表格格式化** | 将杂乱的文本一键整理为对齐的 Markdown 表格。 | 官方 |
| `[ ]` | `⚡️` | **哈希计算器** | 计算 MD5, SHA-1, SHA-256, Base64 摘要。 | 官方 |
| `[ ]` | `⚡️` | **Slug 生成器** | 将标题文本转换为 URL 友好的 Slug。 | 官方 |
...
| `[ ]` | `⚡️` | **Curl 转代码** | 将 curl 命令转换为 Swift, Python, JS 等语言的请求代码。 | 官方 |
...
| `[ ]` | `⚡️` | **智能日程** | 识别文本中的时间信息，生成日程或 .ics 文件。 | 官方 |
